...
def manageVideoLessons(request):
    ...
    baseContext = {
        "videoLessonObjects": Video.objects.all,
        "videoIDtoUse": "vid%s" % (timestamp),
    ...
    if request.method == "POST":
        ...
        if request.POST["managementAction"] == "create":
            ...
            if Video.objects.filter(videoName=videoName).exists():
                context = {
                    **baseContext,
                    "error": "Sorry, a video with this name already exists, please choose a different name for your video.",
                }
                return render(request, "videoLessons/manage.html", context)
            else:
                if "videoFile" in request.FILES:
                    ...
                    if not re.findall(".*\.mp4$", request.POST["fileName"], flags=re.I):
                        ...
                        context = {
                            **baseContext,
                            "error": "Sorry, your video format was not recognised, please upload the video in .mp4 format.",
                        }
                        return render(request, "videoLessons/manage.html", context)
                    ...
                    if Video.objects.filter(
                        ...
                        context = {
                            **baseContext,
                            "error": "Error, the video ID already exists, please try again",
                        }
                        return render(request, "videoLessons/manage.html", context)
                    ...
                    context = {
                        **baseContext,
                        "error": "No video file uploaded, please select a video file to upload",
                    }
                    return render(request, "videoLessons/manage.html", context)
                ...
                context = {
                    **baseContext,
                    "notification": "Video: %s successfully created!" % (videoName),
                }
                return render(request, "videoLessons/manage.html", context)
        ...
        if request.POST["managementAction"] == "edit":
            ...
            if Video.objects.filter(videoID=videoID).exists():
                ...
                context = {
                    **baseContext,
                    "notification": "Video: %s successfully edited!"
                    % (newVideo.videoName),
                }
                return render(request, "videoLessons/manage.html", context)
...
def viewVideo(request, videoID):
    try:
        ...
        context = {
            "error": "Sorry, this video does not exist",
