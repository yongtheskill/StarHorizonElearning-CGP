...
function autoGradeAdd(qID) {
  ...
  if (question.isAutoGraded) {
    $(
      `div.question-div-identifier-id-${question.questionID.toString()} > div.questionContainer`
    ).append(generateAutoGrade(question));
  } else {
    $(
      `div.question-div-identifier-id-${question.questionID.toString()} > div.questionContainer>.autoGradeContainer`
    ).remove();
  }
  $("select").formSelect();
}
  ...
  let question = questions.find((o, i) => {
    if (o.questionID === qID) {
      questions[i].correctAnswer =
        questions[i].questionOptions[parseInt($(`#validationAnswer${qID}`).val())];
      return true; // stop searching
    }
  });
}
