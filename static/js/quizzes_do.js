...
function saAnsChanged(qID) {
  let question = questionResponses.find((o, i) => {
    if (o.questionID === qID) {
      ...
      if (
        questionResponses[i].isAutoGraded === true &&
        questionResponses[i].studentResponse === questionResponses[i].correctAnswer
      ) {
...
function cbAnsChanged(qID, checkboxIndex) {
  let question = questionResponses.find((o, i) => {
    if (o.questionID === qID) {
      ...
      if (
        questionResponses[i].isAutoGraded === true &&
        JSON.stringify(questionResponses[i].studentCheckboxValues) ===
          JSON.stringify(questionResponses[i].checkboxValues)
      ) {
        questionResponses[i].isCorrect = true;
      } else if (questionResponses[i].isAutoGraded === true) {
        questionResponses[i].isCorrect = false;
...
function renderQuizQuestions(quizJSON) {
  ...
  for (k = 0; k < questionsData.length; k++) {
    ...
    newQuestionResponse = new QuestionResponse(
      questionsData[k].questionType,
      questionsData[k].questionID
    );
  ...
  function generateQuesionHTML(question, index) {
    //actual question
    var generatedHTML = `
        <div class="row questionWrapper z-depth-2 question-div-identifier-id-${
          question.questionID
        }">
            <div class="questionContainer">
                <div class="row  noPadding valign-wrapper">
                    <div class="input-field col s12 quesionInput">
                        <h6 style="margin: 0px;" id="questionNumberLabel${
                          question.questionID
                        }"><span class="qNumLabel">${index + 1}</span>. ${
