...
def classView(request, classId):
    ...
    studentIds = [i.id for i in students]
            ...
            quizData.append(
                {
                    ...
                    "completed": QuizAttempt.objects.filter(
                        quiz__quizID=quiz.quizID, student__id__in=studentIds
                    )
                    .values_list("student", flat=True)
                    .distinct()
                    .count(),
                    "passed": QuizAttempt.objects.filter(
                        quiz__quizID=quiz.quizID,
                        student__id__in=studentIds,
                        score__gte=quiz.passingScore,
                    )
                    .values_list("student", flat=True)
                    .distinct()
                    .count(),
    ...
    context = {
        ...
        "students": students,
        "courses": courses,
        "liveLessonsNew": liveLessonsNew,
        "liveLessonsOld": liveLessonsOld,
        "quizData": quizData,
...
def courseView(request, courseId):
    ...
    modules = list(Module.objects.filter(course=courseId).order_by("moduleName"))
    ...
    for module in modules:
        ...
        module.lessonsAll = [{"lesson": l, "new": True} for l in lessonsNew]
        module.lessonsAll.extend([{"lesson": l, "new": False} for l in lessonsOld])
...
def classListView(request):
    ...
    context = {
        "classes": classes,
        "outstandingQuizzes": outstandingQuizzes,
...
def createClass(request):
    ...
    if request.method == "POST":
        ...
        context = {
            ...
            "outstandingQuizzes": outstandingQuizzes,
            "outstandingLivelessons": outstandingLivelessons,
            "notification": "Class successfully uploaded!",
        }
        ...
        return render(
            request,
            "accountManagement/createClass.html",
            {
...
def forgotPassword(request):
    if request.method == "POST":
        try:
            ...
            return render(
                request,
                "accountManagement/resetNotification.html",
                {
                    "mainText": "Please check your email to reset your password.",
                    "secText": "The email will only be sent if an account corresponding to your username exists.",
                },
            )

        user.pwResetCode = "".join(
            random.choice(string.ascii_letters) for i in range(64)
        )

        user.pwResetTime = sgt.localize(datetime.now()) + timedelta(minutes=15)
        ...
        html_message = render_to_string(
            "accountManagement/pwreset.html",
            {
...
def forgotPasswordReset(request, passwordCode):
    try:
        ...
        return render(
            request,
            "accountManagement/resetNotification.html",
            {"mainText": "Password reset link is invalid."},
        )
    ...
    if sgt.localize(datetime.now()) > user.pwResetTime:
        return render(
            request,
            "accountManagement/resetNotification.html",
            {"mainText": "Password reset link has expired."},
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "accountManagement/resetNotification.html",
                {"mainText": "Password successfully reset."},
            )
        else:
